...
(function (a, b) {
    ...
    function g(a, b) {
        var c = a.element.classList, d = y.state.scrolling(b);
        c.contains(d) ? clearTimeout(z[b]) : c.add(d)
    }

    function h(a, b) {
        z[b] = setTimeout(function () {
            return a.isAlive && a.element.classList.remove(y.state.scrolling(b))
    ...
    function t(a, b) {
        function c(b) {
            b.touches && b.touches[0] && (b[j] = b.touches[0]["page" + o.toUpperCase()]), E === l && (r[n] = s + u * (b[j] - t), g(a, o), q(a), b.stopPropagation(), b.preventDefault())
        }

        function d() {
            h(a, o), a[p].classList.remove(y.state.clicking), document.removeEventListener("mousemove", c), document.removeEventListener("mouseup", d), document.removeEventListener("touchmove", c), document.removeEventListener("touchend", d), E = null
        }

        function e(b) {
            null === E && (E = l, s = r[n], b.touches && (b[j] = b.touches[0]["page" + o.toUpperCase()]), t = b[j], u = (a[i] - a[f]) / (a[k] - a[m]), b.touches ? (document.addEventListener("touchmove", c, {passive: !1}), document.addEventListener("touchend", d)) : (document.addEventListener("mousemove", c), document.addEventListener("mouseup", d)), a[p].classList.add(y.state.clicking)), b.stopPropagation(), b.cancelable && b.preventDefault()
        }

        var f = b[0], i = b[1], j = b[2], k = b[3], l = b[4], m = b[5], n = b[6], o = b[7], p = b[8], r = a.element,
            s = null, t = null, u = null;
        a[l].addEventListener("mousedown", e), a[l].addEventListener("touchstart", e)
    }/* eslint-disable */
    ...
    C.prototype.eventElement = function (a) {
        var b = this.eventElements.filter(function (b) {
            return b.element === a
        })[0];
        return b || (b = new A(a), this.eventElements.push(b)), b
    }, C.prototype.bind = function (a, b, c) {
        this.eventElement(a).bind(b, c)
    }, C.prototype.unbind = function (a, b, c) {
    ...
    var D = {
        ...
        "click-rail": function (a) {
        ...
        }, "drag-thumb":// Variable to track the currently active slider
            function (a) {
                t(a, ["containerHeight", "contentHeight", "pageY", "railYHeight", "scrollbarY", "scrollbarYHeight", "scrollTop", "y", "scrollbarYRail"]), t(a, ["containerWidth", "contentWidth", "pageX", "railXWidth", "scrollbarX", "scrollbarXWidth", "scrollLeft", "x", "scrollbarXRail"])
            }, keyboard: function (a) {
            function b(b, d) {
                var e = v(c.scrollTop);
                if (0 === b) {
            ...
            function f(b, c, d) {// FIXME: this is a workaround for <select> issue in FF and IE #571
                if (!D.isWebKit && h.querySelector("select:focus")) return !0;
                if (!h.contains(b)) return !1;
                for (var e = b; e && e !== h;) {
                    if (e.classList.contains(y.element.consuming)) return !0;
            ...
            function g(a) {
                return a.target !== b.scrollbarX && a.target !== b.scrollbarY && !(a.pointerType && "pen" === a.pointerType && 0 === a.buttons) && (!!(a.targetTouches && 1 === a.targetTouches.length) || !!(a.pointerType && "mouse" !== a.pointerType && a.pointerType !== a.MSPOINTER_TYPE_MOUSE))
            }

            function h(a) {
                if (g(a)) {
                    var b = f(a);
                    m.startOffset.pageX = b.pageX, m.startOffset.pageY = b.pageY, m.startTime = new Date().getTime(), null !== m.easingLoop && clearInterval(m.easingLoop)
                }
            }
            ...
            function j(a) {
                if (g(a)) {
                    var b = f(a), e = {pageX: b.pageX, pageY: b.pageY}, h = e.pageX - m.startOffset.pageX,
                        j = e.pageY - m.startOffset.pageY;
                    if (i(a.target, h, j)) return;
                    d(h, j), m.startOffset = e;
                    var k = new Date().getTime(), l = k - m.startTime;
                    0 < l && (m.speed.x = h / l, m.speed.y = j / l, m.startTime = k), c(h, j) && a.cancelable && a.preventDefault()
                }
            }

            function k() {
                b.settings.swipeEasing && (clearInterval(m.easingLoop), m.easingLoop = setInterval(function () {
                    return b.isInitialized ? void clearInterval(m.easingLoop) : m.speed.x || m.speed.y ? .01 > u(m.speed.x) && .01 > u(m.speed.y) ? void clearInterval(m.easingLoop) : void (d(30 * m.speed.x, 30 * m.speed.y), m.speed.x *= .8, m.speed.y *= .8) : void clearInterval(m.easingLoop)
                }, 10))
            }

            if (D.supportsTouch || D.supportsIePointer) {
                var l = b.element, m = {startOffset: {}, startTime: 0, speed: {}, easingLoop: null};
                D.supportsTouch ? (b.event.bind(l, "touchstart", h), b.event.bind(l, "touchmove", j), b.event.bind(l, "touchend", k)) : D.supportsIePointer && (window.PointerEvent ? (b.event.bind(l, "pointerdown", h), b.event.bind(l, "pointermove", j), b.event.bind(l, "pointerup", k)) : window.MSPointerEvent && (b.event.bind(l, "MSPointerDown", h), b.event.bind(l, "MSPointerMove", j), b.event.bind(l, "MSPointerUp", k)))
            }
        }/* eslint-disable */
    }, H = function (d, e) {
        var f = this;
        ...
        for (var g in this.element = d, d.classList.add(y.main), this.settings = F(), e) this.settings[g] = e[g];
        this.containerWidth = null, this.containerHeight = null, this.contentWidth = null, this.contentHeight = null;
        var h = function () {
            return d.classList.add(y.state.focus)
        }, i = function () {
            return d.classList.remove(y.state.focus)
        };
        this.isRtl = "rtl" === a(d).direction, !0 === this.isRtl && d.classList.add(y.rtl), this.isNegativeScroll = function () {
            var a = d.scrollLeft, b = null;
            return d.scrollLeft = -1, b = 0 > d.scrollLeft, d.scrollLeft = a, b
        }(), this.negativeScrollAdjustment = this.isNegativeScroll ? d.scrollWidth - d.clientWidth : 0, this.event = new C, this.ownerDocument = d.ownerDocument || document, this.scrollbarXRail = c(y.element.rail("x")), d.appendChild(this.scrollbarXRail), this.scrollbarX = c(y.element.thumb("x")), this.scrollbarXRail.appendChild(this.scrollbarX), this.scrollbarX.setAttribute("tabindex", 0), this.event.bind(this.scrollbarX, "focus", h), this.event.bind(this.scrollbarX, "blur", i), this.scrollbarXActive = null, this.scrollbarXWidth = null, this.scrollbarXLeft = null;
        var j = a(this.scrollbarXRail);
        this.scrollbarXBottom = parseInt(j.bottom, 10), isNaN(this.scrollbarXBottom) ? (this.isScrollbarXUsingBottom = !1, this.scrollbarXTop = n(j.top)) : this.isScrollbarXUsingBottom = !0, this.railBorderXWidth = n(j.borderLeftWidth) + n(j.borderRightWidth), b(this.scrollbarXRail, {display: "block"}), this.railXMarginWidth = n(j.marginLeft) + n(j.marginRight), b(this.scrollbarXRail, {display: ""}), this.railXWidth = null, this.railXRatio = null, this.scrollbarYRail = c(y.element.rail("y")), d.appendChild(this.scrollbarYRail), this.scrollbarY = c(y.element.thumb("y")), this.scrollbarYRail.appendChild(this.scrollbarY), this.scrollbarY.setAttribute("tabindex", 0), this.event.bind(this.scrollbarY, "focus", h), this.event.bind(this.scrollbarY, "blur", i), this.scrollbarYActive = null, this.scrollbarYHeight = null, this.scrollbarYTop = null;
